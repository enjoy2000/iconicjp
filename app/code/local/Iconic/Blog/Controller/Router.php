...
class Iconic_Blog_Controller_Router extends Mage_Core_Controller_Varien_Router_Abstract
{
    public function initControllerRouters($observer)
    {
        ...
        $blog = new Iconic_Blog_Controller_Router();
    ...
    public function match(Zend_Controller_Request_Http $request)
    {
        ...
        $identifier = str_replace(Mage::helper('blog')->getBlogitemUrlSuffix(), '', $identifier);

        if (substr($request->getPathInfo(), 0, 7) !== '/blog') {
            if ($identifier == '') {
                ...
                $request->setModuleName('blog')
                ->setControllerName('blogitem')
                    ->setActionName('print')
                    ->setParam('article', $param);
                return true;
            } elseif (substr($identifier, 0, 9) === 'blogitem/') {
                $str = str_replace('blogitem/view/id/', '', $identifier);
                $len = strcspn($identifier, '/');
                ...
                ->setControllerName('index')
                    ...
                    ->setParam('q', $param);
                return true;
            } elseif (substr($identifier, 0, 15) !== 'adminhtml_blog/' && strpos($identifier, '/')
