...
class Iconic_Blog_Adminhtml_BlogController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function indexAction() {
        $this->_initAction()
            ->_addContent($this->getLayout()->createBlock('blog/adminhtml_blog'))
            ->renderLayout();
    }
    ...
    public function editAction() {
        ...
        if ($id) {
            ...
            if (! $model->getId()) {
                Mage::getSingleton('adminhtml/session')
                    ->addError(Mage::helper('blog')->__('Blog article does not exist'));
                $this->_redirect('*/*/');
                return;
            }
        }
        ...
        $this->_initAction()
            ->_addContent($this->getLayout()->createBlock('blog/adminhtml_blog_edit'))
    ...
    public function saveAction() {
        if ($data = $this->getRequest()->getPost()) {
            ...
            $arr = array();
            $arr = $blogCollection->getData();
            if (isset($arr[0]) && $this->getRequest()->getParam('id') == $arr[0]['blog_id'] && $data['url_key'] == $arr[0]['url_key']) {
                $sameUrl = null;
            } else {
                $sameUrl = $blogCollection->getData();
            }
            if ($sameUrl != null) {
                Mage::getSingleton('adminhtml/session')->addError(Mage::helper('blog')->__('Blog Item with such url_key already exists'));
                ...
                if (isset($_FILES['image_short_content']['name']) && ($_FILES['image_short_content']['name'] != '')
                    ...
                    if ($data['short_height_resize'] && $data['short_width_resize']) {
                        $resizePath = Mage::getBaseDir('media') . DS . 'blog' . DS . $data['short_width_resize'] . 'x' . $data['short_height_resize'] . DS;
                    }
                    $result = unlink($resizePath . str_replace('blog/', '', $data['image_short_content']['value']));
                ...
                if (isset($_FILES['image_full_content']['name']) && ($_FILES['image_full_content']['name'] != '')
                    ...
                    if ($data['full_height_resize'] && $data['full_width_resize']) {
                        $resizePath = Mage::getBaseDir('media') . DS . 'blog' . DS . $data['full_width_resize'] . 'x' . $data['full_height_resize'] . DS;
                    }
                    $result = unlink($resizePath . str_replace('blog/', '', $data['image_full_content']['value']));
                ...
                // prepare dates
                if ($this->getRequest()->getParam('blog_time')!='') {
                    $dateFormatIso = Mage::app()->getLocale()->getDateTimeFormat(Mage_Core_Model_Locale::FORMAT_TYPE_SHORT);
                    if (!Zend_Date::isDate($this->getRequest()->getParam('blog_time') . ' ' . date("H:i:s"), $dateFormatIso)) {
                        throw new Exception($this->__(('Blog date field is required')));
                    }
                    $date = new Zend_Date($this->getRequest()->getParam('blog_time') . ' ' . date("H:i:s"), $dateFormatIso);
                    $dateInfo = $date->toArray();
                    $data['blog_time'] = preg_replace('/([0-9]{4})\-(.*)/', $dateInfo['year'].'-$2', $date->toString('YYYY-MM-dd HH:mm:ss'));
                } else {
                    $data['blog_time'] = new Zend_Db_Expr('null');
                }
                ...
                try {
                    if ($this->getRequest()->getParam('blog_time') == NULL) {
                        $model->setBlogTime(now());
                        $model->setCreatedTime(now());
                    } else {
                        if (isset($arr[0]) && (!$blogItemId = $arr[0]['blog_id'])) {
                            $model->setCreatedTime(now());
                        }
                    }
                    ...
                    Mage::getSingleton('adminhtml/session')
                        ->addSuccess(Mage::helper('blog')->__('Blog article has been successfully saved'));
                ...
                Mage::getSingleton('adminhtml/session')->addError(Mage::helper('blog')->__('No items to save'));
                $this->_redirect('*/*/');
            }
        }
    }
        ...
        if (!is_array($blogIds)) {
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session')
                    ...
                    ->__('%d record(s) have been successfully deleted', count($blogIds)));
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
            }
        }
    ...
    public function exportCsvAction()
    {
        $fileName   = 'blog.csv';
        $grid       = $this->getLayout()
            ->createBlock('blog/adminhtml_blog_grid')
            ->addColumn('blog_id', array(
            ...
            'header'    => Mage::helper('blog')->__('Short Content'),
            'type'     =>'text',
            'index'     => 'short_content',
        ))->addColumn('image_short_content', array(
            'header'    => Mage::helper('blog')->__('Short Content Image'),
            'type'     =>'text',
            'display' => 'none',
            'index'     => 'image_short_content',
        ))->addColumn('full_content', array(
            'header'    => Mage::helper('blog')->__('Full Content'),
            'type'     =>'text',
            'index'     => 'full_content',
        ))->addColumn('image_full_content', array(
            'header'    => Mage::helper('blog')->__('Full Content Image'),
            'type'     =>'text',
            'index'     => 'image_full_content',
        ))->addColumn('document', array(
            'header'    => Mage::helper('blog')->__('Document'),
            'type'     =>'text',
            'index'     => 'document',
            ...
            'header'    => Mage::helper('blog')->__('Tags'),
            'type'     =>'text',
            'index'     => 'tags',
        ));
    ...
    public function massStatusAction()
    {
        ...
        if (!is_array($blogIds)) {
            ...
            try {
                ...
                $this->_getSession()
                    ->addSuccess($this->__('%d record(s) have been successfully updated', count($blogIds)));
            } catch (Exception $e) {
